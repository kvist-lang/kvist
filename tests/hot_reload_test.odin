...
@(test)
olive_resource_watch_ignore_skips_exact_directory_names :: proc(t: ^testing.T) {
    root, dir_err := os.make_directory_temp("", "kvist-olive-resource-ignore-*", context.allocator)
    ...
    _, path, found := olive_reload.newest_resource_write_time([]string{root}, []string{".worktrees"})
    if found {
        delete(path)
    }
    testing.expect_value(t, found, false)

    _, path_without_ignores, found_without_ignores := olive_reload.newest_resource_write_time([]string{root})
    ...
    _, exact_path, found_exact_only := olive_reload.newest_resource_write_time([]string{root}, []string{".worktrees"})
